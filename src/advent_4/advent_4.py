...
def inclusive_between(value, lower, upper):
    return value >= lower and value <= upper


def validate_year(lower_limit: int = 0, upper_limit: int = 0, value: str = "") -> bool:
    try:
        if len(value) != 4:
            return False
        int_value = int(value)
        return inclusive_between(int_value, lower_limit, upper_limit)
    except ValueError:
        return False


def validate_byr(value: str) -> bool:
    return validate_year(lower_limit=1920, upper_limit=2002, value=value)


def validate_iyr(value: str) -> bool:
    return validate_year(lower_limit=2010, upper_limit=2020, value=value)


def validate_eyr(value: str) -> bool:
    return validate_year(lower_limit=2020, upper_limit=2030, value=value)
...
def validate_hgt(value: str) -> bool:
    ...
    if value.endswith("cm"):
        return inclusive_between(int_value, 150, 193)
    if value.endswith("in"):
        return inclusive_between(int_value, 59, 76)
    return False


def validate_hcl(value: str) -> bool:
    return len(value) == 7 and len(re.findall(r"#[0-9a-f]{6}", value)) > 0
...
def validate_pid(value: str) -> bool:
    return len(value) == 9 and len(re.findall(r"[0-9]{9}", value)) > 0
    ...
    def is_valid(self) -> bool:
        for field in FIELDS:
            ...
            if field not in self:
                return False
            if not FIELDS[field](self[field]):
                return False
        return True


def add_fields_to_passport(passport: Passport, line: str) -> Passport:
    fields = line.split(" ")
    for field in fields:
...
def parse_passports(lines: str) -> list:
    passport = Passport()
    ...
    for line in lines.split("\n"):
        ...
        if not line:
            ...
            add_fields_to_passport(passport, line)
    return passports
...
passports: list = parse_passports(input)
valid_count: int = count_valid_passports(passports)

print("There are {} valid passports".format(valid_count))
