...
def get_bounds(unparsed_bounds: str):
    parts = unparsed_bounds.split("-")
    lower_bound = int(parts[0])
    upper_bound = int(parts[1])
    return lower_bound - 1, upper_bound - 1


def letter_in_index(string, index, letter):
    try:
...
def is_valid_password(password: str) -> bool:
    ...
    lower_index, upper_index = get_bounds(parts[0])
    letter = parts[1][0]
    string = parts[2]

    tries = 0
    if letter_in_index(string, lower_index, letter):
        tries += 1
    if letter_in_index(string, upper_index, letter):
        tries += 1
    return 1 == tries
...
print("Valid passwords {}".format(valid_passwords_count))
